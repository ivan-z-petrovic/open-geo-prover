...
public class RatioOfTwoCollinearSegments {
	...
	public void transformToAlgebraicForm() {
		...
		if (xA.getVariableType() == xB.getVariableType() && xA.getIndex() == xB.getIndex()) {
			...
			this.numerator = OGPCP.instantiateCondition(yNumerator, pointsMap);
			this.denominator = OGPCP.instantiateCondition(yDenominator, pointsMap);
		}
		else {
			// use x-coordinates
			this.numerator = OGPCP.instantiateCondition(xNumerator, pointsMap);
			this.denominator = OGPCP.instantiateCondition(xDenominator, pointsMap);
		}
	}
