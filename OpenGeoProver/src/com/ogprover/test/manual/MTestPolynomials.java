...
public class MTestPolynomials {
	...
	/**
	 * Verification of correctness of polynomial multiplying result
	 * for multithread case
	 * 
	 * @param res	Polynomial with result
	 * @return		TRUE or FALSE depending whether result is valid or not.
	 */
	public static boolean verifyProductResult(Polynomial res){
		Collection<Term> col = res.getTerms().values(); // values in ascending order
		...
		while (it.hasNext()){
			...
			double coeffDiff = 0;
			if (ii <= 500)
				coeffDiff = ((UTerm)curr).getCoeff() - (ii + 1);
			else
				coeffDiff = ((UTerm)curr).getCoeff() - (1000 - ii + 1);
			if (coeffDiff <= -OGPConstants.EPSILON || 
				coeffDiff >= OGPConstants.EPSILON || 
				(ii > 0 && curr.getPowers().get(0).getExponent() != ii)) {
				isValid = false;
				break;
			}
			ii++;
	...
	/**
	 * Execution of multithreaded polynomial multiplying.
	 * 
	 * @param n		Number of parallel threads.
	 */
	public static void launchOnePolyMultTest(int n){
		Stopwatch s = new Stopwatch();
		String timeMessage = null;
		
		/* 
		 * Multi thread case with n threads
		 */
		Polynomial p = new UPolynomial();
		Polynomial q = null;
		
		// p = 1 + (u_1) + (u_1)^2 + (u_1)^3 + ... + (u_1)^500
		for (int ii = 0; ii <= 500; ii++) {
			Term t = new UTerm(1);
			if (ii > 0)
				t.addPower(new Power(Variable.VAR_TYPE_UX_U, 1, ii));
			p.addTerm(t);
		}
		// q = p
		q = p.clone();
		// q = p^2
		s.startMeasureTime();
		q.multiplyByPolynomialConcurrently(p, n);
		s.endMeasureTime();
		timeMessage = "Polynomials multiplied in " + s.getTimeIntSec() + " seconds.";
		
		// verify result
		boolean valid = verifyProductResult(q);
		System.out.println("Multi-thread (with " + n + " threads) multiplying:");
		if (valid)
			System.out.println("Polynomials successfully multiplied. " + timeMessage);
		else
			System.out.println("Polynomials unsuccessfully multiplied. " + timeMessage);
		System.out.println();
	}
	
	/**
	 * Testing polynomial multiplying in various scenarios.
	 */
	public static void testPolynomialsMultiplying() {
		/* 
		 * Single thread case
		 */
		launchOnePolyMultTest(1);	
		/* 
		 * Multi thread case with 2 threads
		 */
		launchOnePolyMultTest(2);
		/* 
		 * Multi thread case with 3 threads
		 */
		launchOnePolyMultTest(3);
		/* 
		 * Multi thread case with 4 threads
		 */
		launchOnePolyMultTest(4);
		/* 
		 * Multi thread case with 5 threads
		 */
		launchOnePolyMultTest(5);
		/* 
		 * Multi thread case with 6 threads
		 */
		launchOnePolyMultTest(6);
		/* 
		 * Multi thread case with 7 threads
		 */
		launchOnePolyMultTest(7);
		/* 
		 * Multi thread case with 8 threads
		 */
		launchOnePolyMultTest(8);
		/* 
		 * Multi thread case with 9 threads
		 */
		launchOnePolyMultTest(9);
		/* 
		 * Multi thread case with 10 threads
		 */
		launchOnePolyMultTest(10);
		/* 
		 * Multi thread case with 20 threads
		 */
		launchOnePolyMultTest(20);
		/* 
		 * Multi thread case with 30 threads
		 */
		launchOnePolyMultTest(30);
		/* 
		 * Multi thread case with 40 threads
		 */
		launchOnePolyMultTest(40);
		/* 
		 * Multi thread case with 50 threads
		 */
		launchOnePolyMultTest(50);
		/* 
		 * Multi thread case with 60 threads
		 */
		launchOnePolyMultTest(60);
		/* 
		 * Multi thread case with 70 threads
		 */
		launchOnePolyMultTest(70);
		/* 
		 * Multi thread case with 80 threads
		 */
		launchOnePolyMultTest(80);
		/* 
		 * Multi thread case with 90 threads
		 */
		launchOnePolyMultTest(90);
		/* 
		 * Multi thread case with 100 threads
		 */
		launchOnePolyMultTest(100);
	}
	
	/**
	 * Testing of polynomials multiplying in loop
	 * 
	 * @param n	Number of parallel threads
	 */
	public static void testPolynomialsMultiplyingInLoop(int n) {
		if (n <= 0 || n > 50)
			System.out.println("Wrong number of iterations passed in.");
		else {
			for (int ii = 0; ii < n; ii++) {
				System.out.printf("========== Step %d ==========\n", ii+1);
				testPolynomialsMultiplying();
				System.out.println();
			}
		}
	}
