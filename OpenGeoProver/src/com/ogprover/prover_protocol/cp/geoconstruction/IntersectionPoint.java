...
public class IntersectionPoint extends Point {
	...
	public boolean isValidConstructionStep() {
		...
		try {
			...
			if (((GeoConstruction)this.firstPointSet).getIndex() < 0 || ((GeoConstruction)this.firstPointSet).getIndex() >= this.index ||
				...
				output.openItemWithDesc("Error: ");
				output.closeItemWithDesc("Intersection point " + this.getGeoObjectLabel() + " can't be constructed since some base points' set is not yet constructed or not added to construction protocol");
				return false;
			}
		} catch (IOException e) {
	 ...
	 * temporary point is added to construction protocol and it has to be transformed
