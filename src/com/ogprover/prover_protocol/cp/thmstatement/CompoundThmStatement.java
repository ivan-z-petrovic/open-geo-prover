...
public abstract class CompoundThmStatement extends ThmStatement {
	 ...
	 */
	/**
	 * @see com.ogprover.prover_protocol.cp.thmstatement.ThmStatement#isValid()
	 */
	@Override
	public boolean isValid() {
		// Compound geometry theorem statement is valid iff all its particle
		// statements that appear in it are valid within same construction protocol.
		...
		for (ThmStatement statement : this.particleThmStatements) {
			if (this.consProtocol != statement.getConsProtocol()) {
				OpenGeoProver.settings.getLogger().error("Found particle statement not related to construction protocol of this compound statement.");
				return false;
			}
